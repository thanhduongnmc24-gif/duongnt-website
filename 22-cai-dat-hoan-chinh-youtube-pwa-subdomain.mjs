...
export default function DuongTube() {
  ...
  async function search(e:FormEvent){e.preventDefault();if(!q.trim())return;setLoading(true);setMsg("");try{const r=await fetch(\`/api/youtube/search?q=\${encodeURIComponent(q.trim())}\`);const k=await r.json();if(!r.ok)throw new Error(k.loi||"Không thể tìm kiếm");setItems(k.items||[]);if(!k.items?.length)setMsg("Không tìm thấy video.")}catch(error){setMsg(error instanceof Error?error.message:"Có lỗi xảy ra") }finally{setLoading(false)}}
  return <main className="min-h-screen bg-slate-950 text-white"><header className="sticky top-0 z-50 border-b border-white/10 bg-slate-950/95 p-3 backdrop-blur"><div className="mx-auto flex max-w-7xl items-center gap-3"><b className="text-xl text-red-500">DuongTube</b><form onSubmit={search} className="flex min-w-0 flex-1"><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Tìm kiếm YouTube..." className="h-11 min-w-0 flex-1 rounded-l-full border border-slate-600 bg-slate-900 px-5 outline-none focus:border-blue-500"/><button disabled={loading} className="rounded-r-full bg-slate-700 px-5 font-bold disabled:opacity-50">{loading?"...":"Tìm"}</button></form></div></header><div className="mx-auto grid max-w-7xl gap-5 p-4 lg:grid-cols-[minmax(0,1fr)_380px]"><section><div className="aspect-video overflow-hidden rounded-2xl bg-black">{video?<iframe key={video.id} src={\`https://www.youtube.com/embed/\${video.id}?autoplay=1&playsinline=1&enablejsapi=1&origin=\${encodeURIComponent(location.origin)}\`} title={video.title} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen referrerPolicy="strict-origin-when-cross-origin" className="h-full w-full"/>:<div className="flex h-full items-center justify-center text-slate-400">Chọn video từ kết quả tìm kiếm</div>}</div>{video?<div className="py-4"><h1 className="text-xl font-black">{video.title}</h1><p className="text-slate-400">{video.channel}</p></div>:null}<p className="rounded-xl border border-amber-400/20 bg-amber-400/10 p-3 text-sm text-amber-200">Trình phát YouTube chính thức. Phát nền hoặc khóa màn hình phụ thuộc YouTube, trình duyệt và hệ điều hành.</p></section><aside className="space-y-2">{msg?<p className="rounded-xl bg-slate-900 p-4 text-slate-300">{msg}</p>:null}{items.map(v=><button key={v.id} onClick={()=>setVideo(v)} className="grid w-full grid-cols-[150px_1fr] gap-3 rounded-xl p-2 text-left hover:bg-white/10"><img src={v.thumbnail} alt="" className="aspect-video rounded-lg object-cover"/><span><strong className="line-clamp-2 text-sm">{v.title}</strong><small className="mt-2 block text-slate-400">{v.channel}</small></span></button>)}</aside></div></main>;
}
`);
...
console.log("5. Khong cam ket phat khi khoa man hinh voi YouTube IFrame.");
