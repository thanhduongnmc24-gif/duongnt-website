...
export function useYouTubePlayer({ onStarted }: { onStarted?: (video: Video) => void } = {}) {
  ...
  const createPlayer = useCallback((api: YouTubeApi, firstVideo: Video) => {
    const host = hostRef.current;
    if (!host || iframePlayerRef.current) return;
    ...
    const created = new api.Player(host, {
      ...
      playerVars: {
        autoplay: 1, controls: 1, disablekb: 0, fs: 1, iv_load_policy: 3,
    ...
    if (isYouTubePlayer(created)) iframePlayerRef.current = created;
  }, []);
  ...
  useEffect(() => {
    ...
    void loadApi().then(api => {
      if (disposed) return;
      apiRef.current = api;
      if (pendingRef.current) createPlayer(api, pendingRef.current);
    }).catch(() => {
  ...
  const resume = useCallback(() => {
    ...
    if (readyRef.current && isYouTubePlayer(player)) player.playVideo();
  }, []);
  ...
  const play = useCallback((video: Video, videos?: Video[], force = false) => {
    ...
    if (readyRef.current && isYouTubePlayer(player)) {
      ...
      player.playVideo();
    } else if (!player && apiRef.current) {
      createPlayer(apiRef.current, video);
    }
  }, [createPlayer, resume]);

  const pause = useCallback(() => {
    const player = iframePlayerRef.current;
    if (readyRef.current && isYouTubePlayer(player)) player.pauseVideo();
    setIsLoading(false);
  }, []);
  ...
  return {
    hostRef, current, queue, isPlaying, isLoading, error, position, duration, volume, repeat,
