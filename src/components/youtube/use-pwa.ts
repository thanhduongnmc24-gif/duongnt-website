...
export function usePwa() {
  ...
  useEffect(() => {
    ...
    display.addEventListener("change", syncDisplayMode);
    ...
    return () => { disposed = true; window.removeEventListener("beforeinstallprompt", beforeInstall); window.removeEventListener("appinstalled", onInstalled); display.removeEventListener("change", syncDisplayMode); };
  }, []);
  ...
  return { canInstall, installed, standalone, updateAvailable, error, install, update };
}
