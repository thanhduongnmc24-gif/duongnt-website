...
export default function DuongTube() {
  ...
  return (
    <div className={`yt-app ${sidebar ? "yt-sidebar-open" : ""} ${player.current ? "yt-has-player" : ""}`}>
      ...
      <main className="yt-content" id="youtube-content">
        ...
        {expanded && activeVideo && <section className="yt-watch" aria-label="Đang phát">
          ...
          <div className="yt-watch-columns"><div>
            ...
            <h1>{activeVideo.title}</h1><div className="yt-watch-meta"><div className="yt-channel"><span className="yt-channel-avatar">{activeVideo.channel[0] || "♪"}</span><strong>{activeVideo.channel}</strong></div><div className="yt-watch-actions"><button className={currentLiked ? "selected" : ""} aria-pressed={currentLiked} onClick={() => toggleLike(activeVideo)}><Heart size={18} fill={currentLiked ? "currentColor" : "none"} />{currentLiked ? "Đã thích" : "Yêu thích"}</button>{!videoMode && !player.isFullscreen && <button className="yt-background-action" disabled={!player.isReady} onClick={() => void player.enterFullscreen()}><MonitorPlay size={18} />{player.isReady ? "Toàn màn hình" : "Đang chuẩn bị"}</button>}<button className={videoMode ? "yt-background-action" : ""} onClick={() => videoMode ? listen(activeVideo) : enableVideo()}>{videoMode ? <Headphones size={18} /> : <MonitorPlay size={18} />}{videoMode ? "Nghe trong nền" : "Xem bằng YouTube"}</button></div></div>
      ...
      <dialog ref={dialogRef} className="yt-help" onClose={() => setHelp(false)} onClick={event => { if (event.target === dialogRef.current) setHelp(false); }}><div className="yt-help-heading"><span className="yt-logo"><Play fill="currentColor" size={20} /></span><h2>DuongTube, luôn bên bạn</h2><button className="yt-icon-button" aria-label="Đóng hướng dẫn" onClick={() => setHelp(false)}><X /></button></div><div className="yt-help-body"><h3><ArrowDownToLine size={21} />Cài đặt ứng dụng</h3><p>Android / máy tính: chọn <b>Cài ứng dụng</b> hoặc mục cài đặt trong menu trình duyệt.</p><p>iPhone / iPad: mở bằng Safari, chọn <b>Chia sẻ → Thêm vào Màn hình chính → Thêm</b>.</p><h3><MonitorPlay size={21} />Xem video</h3><p>Chạm vào ảnh hoặc tên video để phát bằng trình phát nhúng chính thức của YouTube.</p><h3><Headphones size={21} />Nghe khi khóa màn hình</h3><p>Chọn nút tai nghe cạnh video hoặc nút <b>Nghe trong nền</b>. Nếu iframe chưa kịp mở toàn màn hình, đợi nút <b>Đang chuẩn bị</b> đổi thành <b>Toàn màn hình</b> rồi chạm nút đó.</p><p>Khóa màn hình, kéo bảng điều khiển nhạc xuống rồi bấm <b>Play</b>. Chrome sẽ điều khiển trực tiếp phiên media của iframe YouTube.</p><h3><Heart size={21} />Thư viện của riêng bạn</h3><p>Yêu thích và lịch sử được lưu trên thiết bị này. Bạn không cần đăng nhập.</p>{pwa.error && <p role="status">{pwa.error}</p>}{pwa.canInstall && <button className="yt-primary" onClick={pwa.install}><ArrowDownToLine size={18} />Cài DuongTube</button>}</div></dialog>
    </div>
  );
}
