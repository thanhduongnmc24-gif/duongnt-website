...
export default function DuongTube() {
  ...
  useEffect(() => {
    if ("serviceWorker" in navigator) {
      navigator.serviceWorker.register("/youtube-sw.js").catch(console.error);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    audio.play().catch(() => {
      setMsg("Trình duyệt chặn tự phát. Hãy nhấn nút Play một lần để tiếp tục.");
    });
  }, [streamUrl]);
  ...
  useEffect(() => {
    ...
    safeSetAction("play", async () => {
      ...
      try {
        ...
        setMsg("Không thể tiếp tục phát. Hãy mở lại DuongTube và nhấn Play.");
      }
    });
  ...
  function syncMediaSessionPosition() {
    ...
    try {
      ...
      // setPositionState chưa có trên một số WebView/trình duyệt cũ.
    }
  }
  ...
  async function playVideo(v: Video) {
    ...
    try {
      const res = await fetch(`/api/youtube/stream?id=${encodeURIComponent(v.id)}`, {
        cache: "no-store",
      });
      const text = await res.text();

      let data: { url?: string; loi?: string };
      try {
      ...
      if (!res.ok || !data.url) {
        throw new Error(data.loi || "Không thể lấy luồng âm thanh.");
      }
  ...
  async function search(e: FormEvent) {
    ...
    try {
      ...
      let data: { items?: Video[]; loi?: string };
      try {
        data = JSON.parse(text);
      } catch {
      ...
      if (!r.ok) throw new Error(data.loi || "Không thể tìm kiếm.");

      setItems(data.items || []);
      if (!data.items?.length) setMsg("Không tìm thấy video.");
    } catch (error) {
      setMsg(error instanceof Error ? error.message : "Có lỗi xảy ra.");
    } finally {
      setLoading(false);
    ...
    <main className="min-h-screen bg-slate-950 text-white">
      ...
      <div className="mx-auto grid max-w-7xl gap-5 p-4 lg:grid-cols-[minmax(0,1fr)_380px]">
        <section>
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center p-4">
                  ...
                  {streamUrl ? (
                    <audio
                      ...
                      onPlay={handlePlay}
                      onPause={handlePause}
                      onLoadedMetadata={syncMediaSessionPosition}
                      onDurationChange={syncMediaSessionPosition}
                      onTimeUpdate={syncMediaSessionPosition}
                      onRateChange={syncMediaSessionPosition}
                      onEnded={() => {
                        if ("mediaSession" in navigator) {
          ...
          {video ? (
            <div className="py-4">
              ...
              <p className="text-slate-400">{video.channel}</p>
            </div>
          ) : null}

          <div className="space-y-2">
            <p className="rounded-xl border border-emerald-400/20 bg-emerald-400/10 p-3 text-sm text-emerald-300">
              Đã bật Media Session: khi trình duyệt cho phép phát audio nền, bạn có
              thể dùng Play/Pause và tua ngay từ màn hình khóa hoặc tai nghe.
            </p>
            <p className="rounded-xl border border-amber-400/20 bg-amber-400/10 p-3 text-sm text-amber-200">
              Lưu ý: khả năng tiếp tục phát sau khi khóa màn hình còn phụ thuộc
              iOS/Android, trình duyệt và việc URL luồng âm thanh còn hiệu lực.
            </p>
            {msg && video ? (
        ...
        <aside className="space-y-2">
          {msg && !video ? (
            <p className="rounded-xl bg-slate-900 p-4 text-slate-300">{msg}</p>
          ) : null}
          ...
          {items.map((v) => (
            ...
            >
              ...
              <span>
                ...
                <small className="mt-2 block text-slate-400">{v.channel}</small>
              </span>
            </button>
          ))}
        </aside>
