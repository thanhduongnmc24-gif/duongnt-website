...
export default function DuongTube() {
  ...
  return (
    <div className={`yt-app ${sidebar ? "yt-sidebar-open" : ""} ${player.current ? "yt-has-player" : ""}`}>
      ...
      <dialog ref={dialogRef} className="yt-help" onClose={() => setHelp(false)} onClick={event => { if (event.target === dialogRef.current) setHelp(false); }}><div className="yt-help-heading"><span className="yt-logo"><Play fill="currentColor" size={20} /></span><h2>DuongTube, luôn bên bạn</h2><button className="yt-icon-button" aria-label="Đóng hướng dẫn" onClick={() => setHelp(false)}><X /></button></div><div className="yt-help-body"><h3><ArrowDownToLine size={21} />Cài đặt ứng dụng</h3><p>Android / máy tính: chọn <b>Cài ứng dụng</b> hoặc mục cài đặt trong menu trình duyệt.</p><p>iPhone / iPad: mở bằng Safari, chọn <b>Chia sẻ → Thêm vào Màn hình chính → Thêm</b>.</p><h3><MonitorPlay size={21} />Xem video</h3><p>Chạm vào ảnh hoặc tên video để phát bằng trình phát nhúng chính thức của YouTube.</p><h3><Headphones size={21} />Nghe khi khóa màn hình</h3><p>Chọn nút tai nghe bên cạnh video hoặc nút <b>Nghe trong nền</b>, rồi khóa màn hình. Bạn có thể tạm dừng, tua và chuyển bài từ màn hình khóa trên thiết bị hỗ trợ.</p><p>Khả năng phát nền phụ thuộc trình duyệt, hệ điều hành và nguồn âm thanh. Ứng dụng cần mạng để phát nhạc; nếu nguồn bị từ chối, hãy tiếp tục xem bằng trình phát YouTube.</p><h3><Heart size={21} />Thư viện của riêng bạn</h3><p>Yêu thích và lịch sử được lưu trên thiết bị này. Bạn không cần đăng nhập.</p>{pwa.error && <p role="status">{pwa.error}</p>}{pwa.canInstall && <button className="yt-primary" onClick={pwa.install}><ArrowDownToLine size={18} />Cài DuongTube</button>}</div></dialog>
    </div>
  );
}
