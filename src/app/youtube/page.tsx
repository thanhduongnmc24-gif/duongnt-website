...
export default function DuongTube() {
  ...
  const currentLiked = !!player.current && liked.some(v => v.id === player.current?.id);
  ...
  return (
    <div className={`yt-app ${sidebar ? "yt-sidebar-open" : ""} ${player.current ? "yt-has-player" : ""}`}>
      ...
      <main className="yt-content" id="youtube-content">
        ...
        {expanded && player.current && <section className="yt-watch" aria-label="Đang phát">
          ...
          <div className="yt-watch-columns"><div>
            <div className="yt-stage">
              {videoMode ? <iframe title={player.current.title} src={`https://www.youtube.com/embed/${player.current.id}?autoplay=1&playsinline=1&rel=0`} allow="autoplay; encrypted-media; picture-in-picture; fullscreen" allowFullScreen referrerPolicy="strict-origin-when-cross-origin" /> : <><img className="yt-stage-backdrop" src={player.current.thumbnail} alt="" /><div className="yt-stage-art"><img src={player.current.thumbnail} alt="" /><button className="yt-stage-play" onClick={togglePlayback} aria-label={player.isPlaying ? "Tạm dừng" : "Phát nhạc"}>{player.isLoading ? <LoaderCircle className="yt-spin" /> : player.isPlaying ? <Pause fill="currentColor" /> : <Play fill="currentColor" />}</button></div><span className="yt-stage-label"><Headphones size={16} />Chế độ âm thanh</span></>}
            </div>
            <h1>{player.current.title}</h1><div className="yt-watch-meta"><div className="yt-channel"><span className="yt-channel-avatar">{player.current.channel[0] || "♪"}</span><strong>{player.current.channel}</strong></div><div className="yt-watch-actions"><button className={currentLiked ? "selected" : ""} aria-pressed={currentLiked} onClick={() => player.current && toggleLike(player.current)}><Heart size={18} fill={currentLiked ? "currentColor" : "none"} />{currentLiked ? "Đã thích" : "Yêu thích"}</button><button onClick={() => { if (videoMode) { setVideoMode(false); player.toggle(); } else enableVideo(); }}>{videoMode ? <Headphones size={18} /> : <MonitorPlay size={18} />}{videoMode ? "Nghe nhạc" : "Xem video"}</button></div></div>
            <p className="yt-watch-note">{videoMode ? "Chuyển sang Nghe nhạc để dùng trình phát âm thanh và điều khiển trên màn hình khóa." : "Phát nhạc rồi khóa màn hình để tiếp tục nghe trên thiết bị hỗ trợ. Nhạc cần kết nối mạng."}</p>
          </div><div className="yt-watch-queue"><h2><ListMusic size={20} />Tiếp theo</h2>{player.queue.slice(0, 8).map(v => <button key={v.id} className={`yt-queue-item ${v.id === player.current?.id ? "active" : ""}`} onClick={() => { setVideoMode(false); player.play(v, player.queue); }}><img src={v.thumbnail} alt="" /><span><strong>{v.title}</strong><small>{v.channel}</small></span>{v.id === player.current?.id && <Headphones size={17} />}</button>)}</div></div>
        </section>}
        {!expanded && <><div className="yt-section-heading"><div><h1>{heading}</h1>{view === "liked" || view === "history" ? <p>{visible.length} video · Lưu trên thiết bị này</p> : <p>{submitted ? "Khám phá video và nghe theo cách của bạn" : "Khám phá giai điệu cho ngày của bạn"}</p>}</div>{!submitted && view === "home" && <span className="yt-listening-label"><span />Sẵn sàng để nghe</span>}</div>
          {busy ? <div className="yt-grid" aria-label="Đang tải video" aria-busy="true">{Array.from({ length: 9 }, (_, i) => <div className="yt-skeleton" key={i}><div /><span /><span /></div>)}</div> : error ? <div className="yt-empty" role="alert"><WifiOff /><h2>Chưa thể tải video</h2><p>{error}</p><button className="yt-primary" onClick={() => setReload(x => x + 1)}><RefreshCw size={18} />Thử lại</button></div> : !visible.length ? <div className="yt-empty">{view === "liked" ? <Heart /> : view === "history" ? <Clock3 /> : <Search />}<h2>{view === "liked" ? "Những giai điệu bạn muốn giữ lại" : view === "history" ? "Bắt đầu hành trình âm nhạc" : "Không tìm thấy video"}</h2><p>{view === "liked" ? "Nhấn trái tim trên một video để lưu vào thư viện của bạn." : view === "history" ? "Các video bạn nghe sẽ xuất hiện tại đây." : "Thử một từ khóa khác hoặc dán liên kết YouTube."}</p>{(view === "liked" || view === "history") && <button className="yt-primary" onClick={() => navigate("home")}>Khám phá âm nhạc</button>}</div> : <div className="yt-grid">{visible.map(video => <article className={`yt-card ${player.current?.id === video.id ? "yt-card-playing" : ""}`} key={video.id}>
            <button className="yt-thumbnail" onClick={() => play(video)} aria-label={`Phát ${video.title}`}><img src={video.thumbnail || "/youtube-assets/icon.svg"} alt="" loading="lazy" /><span className="yt-card-play"><Play fill="currentColor" size={24} /></span>{player.current?.id === video.id ? <span className="yt-duration yt-now"><Headphones size={13} />{player.isPlaying ? "Đang phát" : "Đã chọn"}</span> : !!video.duration && <span className="yt-duration">{time(video.duration)}</span>}</button>
            <div className="yt-card-info"><span className="yt-channel-avatar">{video.channel[0] || "♪"}</span><div><button className="yt-video-title" onClick={() => play(video)}>{video.title}</button><p>{video.channel}</p>{video.views != null && <p>{views(video.views)}</p>}</div><button className={`yt-like ${liked.some(v => v.id === video.id) ? "selected" : ""}`} onClick={() => toggleLike(video)} aria-label={`${liked.some(v => v.id === video.id) ? "Bỏ thích" : "Yêu thích"} ${video.title}`} aria-pressed={liked.some(v => v.id === video.id)}><Heart size={18} fill={liked.some(v => v.id === video.id) ? "currentColor" : "none"} /></button></div>
          </article>)}</div>}
        </>}
      </main>
      ...
      <dialog ref={dialogRef} className="yt-help" onClose={() => setHelp(false)} onClick={event => { if (event.target === dialogRef.current) setHelp(false); }}><div className="yt-help-heading"><span className="yt-logo"><Play fill="currentColor" size={20} /></span><h2>DuongTube, luôn bên bạn</h2><button className="yt-icon-button" aria-label="Đóng hướng dẫn" onClick={() => setHelp(false)}><X /></button></div><div className="yt-help-body"><h3><ArrowDownToLine size={21} />Cài đặt ứng dụng</h3><p>Android / máy tính: chọn <b>Cài ứng dụng</b> hoặc mục cài đặt trong menu trình duyệt.</p><p>iPhone / iPad: mở bằng Safari, chọn <b>Chia sẻ → Thêm vào Màn hình chính → Thêm</b>.</p><h3><Headphones size={21} />Nghe khi khóa màn hình</h3><p>Chọn bài, nhấn phát trong chế độ <b>Nghe nhạc</b>, rồi khóa màn hình. Bạn có thể tạm dừng, tua và chuyển bài từ màn hình khóa hoặc tai nghe trên thiết bị hỗ trợ.</p><p>Khả năng phát nền phụ thuộc trình duyệt, hệ điều hành và nguồn âm thanh. Ứng dụng cần mạng để phát nhạc; nếu nguồn bị từ chối, hãy dùng <b>Mở YouTube</b> trong trình phát.</p><h3><Heart size={21} />Thư viện của riêng bạn</h3><p>Yêu thích và lịch sử nghe được lưu trên thiết bị này. Bạn không cần đăng nhập.</p>{pwa.error && <p role="status">{pwa.error}</p>}{pwa.canInstall && <button className="yt-primary" onClick={pwa.install}><ArrowDownToLine size={18} />Cài DuongTube</button>}</div></dialog>
    </div>
  );
}
