...
export default async function RootLayout({
  ...
  const laUngDung = laDuongTube || laCongDoan || laLichLamViec;
  const theme = laUngDung ? null : await layThemeWebsite();

  return (
    <html lang="vi" data-theme={laDuongTube ? "duongtube" : laCongDoan ? "congdoan" : laLichLamViec ? "lichlamviec" : theme?.ma}>
      <body
        className={`${inter.className} min-h-screen ${laDuongTube ? "bg-[#0f0f0f] text-[#f1f1f1]" : laCongDoan || laLichLamViec ? "bg-[#f5f7fb] text-[#172033]" : "bg-slate-100 text-slate-900"}`}
        style={laDuongTube ? { backgroundColor: "#0f0f0f", color: "#f1f1f1", colorScheme: "dark" } : laCongDoan || laLichLamViec ? { backgroundColor: "#f5f7fb", color: "#172033", colorScheme: "light" } : undefined}
      >
